...
if ($_SERVER['REQUEST_METHOD'] == "GET")
{
    if (isset ($_GET['domain']))
    {
        ...
        $tGdUser = $domain_properties['gduser'];
        $tGdPass = $domain_properties['gdpass'];
...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if (isset ($_POST['fGdUser'])) $fGdUser = escape_string($_POST['fGdUser']);
    if (isset ($_POST['fGdPass'])) $fGdPass = escape_string($_POST['fGdPass']);
    ...
    $result = db_query ("UPDATE $table_domain SET description='$fDescription',aliases=$fAliases,mailboxes=$fMailboxes,gduser='$fGdUser',gdpass='$fGdPass',maxquota=$fMaxquota,$sqltransport backupmx='$sqlBackupmx',active='$sqlActive',modified=NOW() WHERE domain='$domain'");
    if ($result['rows'] == 1)
    {

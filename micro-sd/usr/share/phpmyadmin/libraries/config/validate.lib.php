...
function validate_server($path, $values)
{
    ...
    if (!$error && $values['Servers/1/auth_type'] == 'config') {
        ...
        $test = test_db_connection($values['Servers/1/extension'], $values['Servers/1/connect_type'], PMA_sanitizeMySQLHost($values['Servers/1/host']), $values['Servers/1/port'], $values['Servers/1/socket'], $values['Servers/1/user'], $password, 'Server');
        if ($test !== true) {
...
function validate_pmadb($path, $values)
{
    ...
    if (!$error) {
        $test = test_db_connection($values['Servers/1/extension'], $values['Servers/1/connect_type'],
            PMA_sanitizeMySQLHost($values['Servers/1/host']), $values['Servers/1/port'], $values['Servers/1/socket'],

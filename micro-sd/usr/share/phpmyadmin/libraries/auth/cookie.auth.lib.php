...
if (function_exists('mcrypt_encrypt')) {
     ...
     *
     * @return  string  the encrypted result as a structure
     *
     * @access  public
     *
     */
    ...
    {
        $iv = PMA_createIV();
        $result = base64_encode(mcrypt_encrypt(MCRYPT_BLOWFISH, $secret, $data, MCRYPT_MODE_CBC, $iv));
	return json_encode(
            array(
                'iv' => base64_encode($iv),
	...
	$newmac = sha1($data['payload'] . $secret);
        ...
        return trim(mcrypt_decrypt(MCRYPT_BLOWFISH, $secret, base64_decode($data['payload']), MCRYPT_MODE_CBC, base64_decode($data['iv'])));
    }
...
function PMA_auth_check()
{
    ...
    if ($GLOBALS['cfg']['AllowArbitraryServer']
     && ! empty($_COOKIE['pmaServer-' . $GLOBALS['server']])) {
        $GLOBALS['pma_auth_server'] = PMA_sanitizeMySQLHost($_COOKIE['pmaServer-' . $GLOBALS['server']]);
    }
